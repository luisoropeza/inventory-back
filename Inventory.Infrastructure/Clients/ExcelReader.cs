...
namespace Inventory.Infrastructure.Clients
{
    ...
    {
        public Task<List<Product>> ReadProductsAsync(Stream stream)
        {
            ...
            foreach (var row in rows)
            {
                ...
                if (!int.TryParse(categoryIdStr, out var catId) || catId <= 0)
                    throw new ArgumentException($"Row '{name}': CategoryId '{categoryIdStr}' is not a valid category ID.");
